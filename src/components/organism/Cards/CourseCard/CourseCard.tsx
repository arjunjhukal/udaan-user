...
export default function CourseCard({ course }: { course: CourseProps }) {
    ...
    return (
        <Box className="course__card rounded-md overflow-hidden" sx={{
            border: `1px solid ${theme.palette.textField.border}`
        }}>
            <Box className="course__image aspect-347/128" sx={{
                background: theme.palette.primary.contrastText
            }}>
                <img src={course.thumbnail_url || "/logo.svg"} alt={course.name} className='w-full h-full object-cover' />
            </Box>
            <Box className="course__content p-3 bg-slate-50 flex flex-col gap-2">
                <div className="over__title flex justify-between items-center">
                    <Typography>{course?.mega_categories && course?.mega_categories[0]}</Typography>
                    <IconButton>
                ...
                <Link to={PATH.COURSE_MANAGEMENT.COURSES.VIEW_COURSE.ROOT(course.id)}>{course.name}</Link>
                <CourseFeature />
                <Divider />
